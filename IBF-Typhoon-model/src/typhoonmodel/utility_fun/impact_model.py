...
class Forecast:
    def __init__(self, remote_dir, countryCodeISO3, admin_level):
        ...
        if TropicalCycloneAdvisoryDomain_events != []:
            ...
            for typhoons in TropicalCycloneAdvisoryDomain_events:
                #
                logger.info(f"Processing data {typhoons}")
                calcuated_wind_fields= self.windfieldData(typhoons) 
                #check if calculated wind fields are empty 
                if calcuated_wind_fields:
                    if not calcuated_wind_fields.empty:                
                        self.typhoon_wind_data[typhoons] = calcuated_wind_fields
                        self.impact_model(typhoon_names=typhoons)
    ...
    def windfieldData(self, typhoons):
        ...
        if tr_HRS != []:
            ...
            coastline = gpd.clip(coast, PHL).to_crs('EPSG:3087')
            points_df = coord_lat.to_crs('EPSG:3087') 
            ...
            YYYYMMDDHH = pd.date_range(
                fcast_data[0].time.values[0], periods=len_ar, freq="H"
            )
            vmax_ = np.ma.mean(
                [var.max_sustained_wind.values[:len_ar] for var in fcast_data],
                axis=0,
            )
        ...
        if MIN_DIST_TO_COAST <200000:            #in meters            
            ...
            return typhhon_wind_data
        else:
            return None

    def impact_model(self, typhoon_names):
        selected_columns = [
            "Mun_Code",
            "storm_id",
            "name",
        ...
        df_hazard = pd.merge(
            self.typhoon_wind_data[typhoon_names],
            self.rainfall_data,
            how="left",
            left_on="adm3_pcode",
            right_on="Mun_Code",
        ...
        df_total["dist_50"] = df_total["HAZ_dis_track_min"].apply(lambda x: 0.01923 if x < 50 else 0)
        ...
        df_impact_forecast = pd.merge(
            df_total[["Damage_predicted", "Mun_Code", "storm_id", "HAZ_dis_track_min"]],
        ...
        impact = df_impact_forecast.groupby("Mun_Code").agg(
            {"Damage_predicted": "mean", "HAZ_dis_track_min": "min"}
        )
        impact_df = pd.merge(
            impact,
            probability_50km,
        ...
        impact_df = impact_df.filter(
            ["Mun_Code", "dist_50", "Damage_predicted", "HAZ_dis_track_min"]
        )
        impact_df.rename(
            columns={
        ...
        impact_df = pd.merge(
            self.pcode, impact_df, how="left", left_on="adm3_pcode", right_on="Mun_Code"
        )
        ...
        probability_impact = df_impact_forecast.groupby("storm_id").agg(
            {"Damage_predicted": "sum"}
        )
        probability_impact.reset_index(inplace=True)
        agg_impact = probability_impact["Damage_predicted"].values
        ...
        json_file_path = (
            self.Output_folder + typhoon_names + "dref_trigger_status" + ".csv"
        )
        ...
        hrs_track_df = self.hrs_track_data[typhoon_names]
        ...
        typhoon_windAll = self.typhoon_wind_data[typhoon_names]
        ...
        typhoon_rainfall = self.rainfall_data[["Mun_Code", "HAZ_rainfall_max_24h"]]
        typhoon_rainfall.rename(
            columns={"Mun_Code": "adm3_pcode", "HAZ_rainfall_max_24h": "rainfall"},
            inplace=True,
        )
        ...
        typhoon_track = self.hrs_track_data[typhoon_names]
        ...
        # dynamic layers
        df_total_upload = df_total_upload.astype(
            {
                "windspeed": "int",
                "rainfall": "int",
                "prob_within_50km": "int",
                "houses_affected": "int",
                "alert_threshold": "int",
                "show_admin_area": "int",
            }
        ...
        for layer in [
            ...
            for ix, row in df_total_upload.iterrows():
                exposure_entry = {"placeCode": row["adm3_pcode"], "amount": row[layer]}
                exposure_place_codes.append(exposure_entry)
